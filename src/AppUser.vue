    ...
    <div class="grid-layout">
      ...
      <aside class="sidebar">
        ...
        <ul class="ship-list">
          ...
          <li v-else v-for="ship in vm.filteredShips" :key="ship.id" @click="onSelectShipFromSidebar(ship)" class="ship-item" :class="{ 'selected': vm.selectedShip && vm.selectedShip.id === ship.id }">
      ...
      <div class="map-container">
        ...
        <SOS mode="user" :ships="vm.ships" :selected-ship="vm.selectedShip" :allowed-sns="vm.extractAccessibleSerials(vm.user)" />
...
export default {
  ...
  methods: {
    onSelectShipFromSidebar(ship) {
      try {
        if (this.vm && typeof this.vm.focusShip === 'function') {
          this.vm.focusShip(ship);
        }
      } finally {
        if (this.vm) this.vm.showSummaryWaypoint = false;
      }
    }
  },
  watch: {
    'vm.selectedShip'(next) {
      if (next && this.vm && this.vm.showSummaryWaypoint) {
        this.vm.showSummaryWaypoint = false;
      }
    }
  }
};
